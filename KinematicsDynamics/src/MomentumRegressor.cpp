...
void MomentumRegressor::compute(const VecX &z, bool compute_derivatives,
  ...
  if (compute_hessian) {
    throw std::invalid_argument(
        "MomentumRegressor: Hessian not implemented yet");
  }
...
#pragma omp parallel for shared(trajPtr_, modelPtr_, jtype, Xtree, Y, pY_pz,   \
  ...
  for (i = 0; i < N; i++) {
    ...
    Eigen::Array<MatRegressor, 1, Eigen::Dynamic> pbodyRegressor_pz(
        trajPtr_->varLength);
      ...
      for (int h = j; h >= 0; h = modelPtr_->parents[h + 1] - 1) {
        ...
        if (modelPtr_->parents[h + 1] > 0) {
          if (compute_derivatives) {
            ...
            for (int k = 0; k < trajPtr_->varLength; k++) {
              pbodyRegressor_pz(k) = Xup(h).transpose() * pbodyRegressor_pz(k) +
                                     dXupdq_Yfull * pq_pz(h, k);
            }
          }

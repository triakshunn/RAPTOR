...
namespace RAPTOR {
// takes in model info, trajectory data and joint type, there is regressor
// generation, but cannot find the optimization problem
IntervalMomentumRegressor::IntervalMomentumRegressor(
  ...
  for (int i = 0; i < modelPtr_->nv; i++) {
    const int pinocchio_joint_id =
        i + 1; // the first joint in pinocchio is the root joint

    // plux in Roy Featherstone's code (transformation matrix from parent to
    // child)
    Xtree(i) = Utils::plux(
        modelPtr_->jointPlacements[pinocchio_joint_id].rotation().transpose(),
  ...
  a_grav << modelPtr_->gravity.angular(), modelPtr_->gravity.linear();
  ...
  // In the original RegressorInverseDynamics, z represents the decision
  // variable, such as coefficients of the polynomial trajectory. Here, z
  // represents the q, q_d themselves since we are using TrajectoryData class.
  ...
  mrPtr_ =
      std::make_shared<MomentumRegressor>(model_input, trajPtr_, jtype_input);
};
...
void IntervalMomentumRegressor::compute(const VecXd &z,
  ...
  int i = 0;
#pragma omp parallel for shared(trajPtr_, modelPtr_, jtype, Xtree, Y, pY_pz,   \
                                    tau, ptau_pz) private(i)                   \
    schedule(dynamic, 1)
  for (i = 0; i < N; i++) {
    ...
    for (int j = 0; j < q_meas.size(); j++) {
      q(j) = q_meas(j) + IntervalHelper::makeErrorInterval(
                             sensor_noise.position_error(j),
                             sensor_noise.position_error_type, q_meas(j));
      q_d(j) = q_d_meas(j) + IntervalHelper::makeErrorInterval(
                                 sensor_noise.velocity_error(j),
                                 sensor_noise.velocity_error_type, q_d_meas(j));
    }

    // below is the extended regressor algorithm from ROAM-Lab
    // refer to
    // https://github.com/ROAM-Lab-ND/spatial_v2_extended/blob/main/v3/dynamics/Hqd_and_CTqd.m
    ...
    Eigen::Array<MatRegressorInt, 1, Eigen::Dynamic> pbodyRegressor_pz(
        2 * modelPtr_->nv);

    // forward pass
    for (int j = 0; j < modelPtr_->nv; j++) {
      const int pinocchio_joint_id =
          j + 1; // the first joint in pinocchio is the root joint
      ...
      if (parent_id > -1) {
        ...
        if (compute_derivatives) {
          ...
          if (j < trajPtr_->Nact) {
            ...
            auto dXupdq_v_parent_id =
                intervalMatrixMultiply(dXupdq(j), v(parent_id));
            for (int k = 0; k < 6; k++) {
      ...
      bodyRegressor << 0, 0, v6, -v5, v1, v2, 0, v3, 0, 0, 0, -v6, 0, v4, 0, v1,
          v2, 0, v3, 0, 0, v5, -v4, 0, 0, 0, 0, v1, v2, v3, v4, 0, -v3, v2, 0,
          0, 0, 0, 0, 0, v5, v3, 0, -v1, 0, 0, 0, 0, 0, 0, v6, -v2, v1, 0, 0, 0,
          0, 0, 0, 0;
      ...
      if (compute_derivatives) {
        for (int k = 0; k < 2 * modelPtr_->nv; k++) {
          ...
          pbodyRegressor_pz(k) << 0, 0, pv6, -pv5, pv1, pv2, 0, pv3, 0, 0, 0,
              -pv6, 0, pv4, 0, pv1, pv2, 0, pv3, 0, 0, pv5, -pv4, 0, 0, 0, 0,
              pv1, pv2, pv3, pv4, 0, -pv3, pv2, 0, 0, 0, 0, 0, 0, pv5, pv3, 0,
              -pv1, 0, 0, 0, 0, 0, 0, pv6, -pv2, pv1, 0, 0, 0, 0, 0, 0, 0;
        }
      }
      ...
      for (int h = j; h >= 0; h = modelPtr_->parents[h + 1] - 1) {
        ...
        if (compute_derivatives) {
          ...
          pSd_pz.row(3) = S(h)(2) * pv_pz(h).row(4) -
                          S(h)(1) * pv_pz(h).row(5) -
                          S(h)(4) * pv_pz(h).row(2) + S(h)(5) * pv_pz(h).row(1);
          pSd_pz.row(4) = S(h)(0) * pv_pz(h).row(5) -
                          S(h)(2) * pv_pz(h).row(3) +
                          S(h)(3) * pv_pz(h).row(2) - S(h)(5) * pv_pz(h).row(0);
          pSd_pz.row(5) = S(h)(1) * pv_pz(h).row(3) -
                          S(h)(0) * pv_pz(h).row(4) -
                          S(h)(3) * pv_pz(h).row(1) + S(h)(4) * pv_pz(h).row(0);
        }
        ...
        if (compute_derivatives) {
          for (int k = 0; k < 2 * modelPtr_->nv; k++) {
            pY_CTv_pz(k).block(i * modelPtr_->nv + h, j * 10, 1, 10) =
                intervalMatrixMultiply(pSd_pz.col(k).transpose(),
                                       bodyRegressor) +
        ...
        if (modelPtr_->parents[h + 1] > 0) {
          if (compute_derivatives) {
            for (int k = 0; k < 2 * modelPtr_->nv; k++) {
              pbodyRegressor_pz(k) = intervalMatrixMultiply(
                  Xup(h).transpose(), pbodyRegressor_pz(k));
            }
            pbodyRegressor_pz(h) +=
                intervalMatrixMultiply(dXupdq(h).transpose(), bodyRegressor);
          }

          bodyRegressor =
              intervalMatrixMultiply(Xup(h).transpose(), bodyRegressor);
        }
      }
    }

    // compute torque
    tau(i) = intervalDoubleMatrixMultiply(
        Y.middleRows(i * modelPtr_->nv, modelPtr_->nv), phi);

    if (compute_derivatives) {
      for (int k = 0; k < 2 * modelPtr_->nv; k++) {
        ptau_pz(i).col(k) = intervalDoubleMatrixMultiply(
            pY_pz(k).middleRows(i * modelPtr_->nv, modelPtr_->nv), phi);
      }
    }
  }
...
#pragma omp parallel for shared(mrPtr_, tau, ptau_pz, Y, pY_pz) private(i)     \
    schedule(dynamic, 1)
    for (i = 0; i < trajPtr_->N; i++) {
      for (int j = 0; j < modelPtr_->nv; j++) {
        const Interval position_error = IntervalHelper::makeErrorInterval(
            sensor_noise.position_error(j), sensor_noise.position_error_type,
            trajPtr_->q(i)(j));
        const Interval velocity_error = IntervalHelper::makeErrorInterval(
            sensor_noise.velocity_error(j), sensor_noise.velocity_error_type,
            trajPtr_->q_d(i)(j));

        // an alternative way to compute the bound of tau using first order
        // Taylor expansion
        ...
        if (IntervalHelper::getRadius(tau_alt) <
            IntervalHelper::getRadius(tau(i)(j))) {
          tau(i)(j) = tau_alt;
        }
        ...
        for (int h = 0; h < 10 * modelPtr_->nv; h++) {
          ...
          Interval Y_CTv_alt =
              Interval(mrPtr_->Y_CTv(i * modelPtr_->nv + j, h));

          for (int k = 0; k < modelPtr_->nv; k++) {
            Y_alt += pY_pz(k)(i * modelPtr_->nv + j, h) * position_error;
            Y_CTv_alt +=
                pY_CTv_pz(k)(i * modelPtr_->nv + j, h) * position_error;
          }
          for (int k = 0; k < modelPtr_->nv; k++) {
            Y_alt += pY_pz(k + modelPtr_->nv)(i * modelPtr_->nv + j, h) *
                     velocity_error;
            Y_CTv_alt +=
                pY_CTv_pz(k + modelPtr_->nv)(i * modelPtr_->nv + j, h) *
                velocity_error;
          }

          if (IntervalHelper::getRadius(Y_alt) <
              IntervalHelper::getRadius(Y(i * modelPtr_->nv + j, h))) {
            Y(i * modelPtr_->nv + j, h) = Y_alt;
          }
          if (IntervalHelper::getRadius(Y_CTv_alt) <
              IntervalHelper::getRadius(Y_CTv(i * modelPtr_->nv + j, h))) {

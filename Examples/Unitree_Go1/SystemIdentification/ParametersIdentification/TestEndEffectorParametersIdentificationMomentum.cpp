...
int main(int argc, char *argv[]) {
  ...
  if (argc < 2) {
    throw std::invalid_argument(
        "No arguments provided. please choose the trajectory file");
  }
  ...
  const std::string friction_parameters_filename =
      folder_name + "friction_params.csv";  // what is the structure of this file? (Need to give true params)
  ...
  if (friction_parameters.size() != 3 * model.nv &&
    ...
    std::cerr << "Friction_parameters size: " << friction_parameters.size()
              << std::endl;
  ...
  try {
    ...
    mynlp->add_trajectory_file(trajectory_filename, sensor_noise, H,
                               TimeFormat::Nanosecond, downsample_rate);

...
EndEffectorIdentificationMomentumPybindWrapper::
    EndEffectorIdentificationMomentumPybindWrapper(
        ...
        const nb_1d_double friction_parameters_input, const int H_input,
        const std::string time_format_string, const bool display_info)
    : H(H_input) {
  // define robot model
  ...
  if (friction_parameters_input.size() != 3 * model.nv &&
    ...
    std::cerr << "Friction_parameters size: "
              << friction_parameters_input.size() << std::endl;
    throw std::runtime_error("Friction solution input is wrong");
  }

  Eigen::VectorXd friction_parameters = Eigen::Map<Eigen::VectorXd>(
      friction_parameters_input.data(), friction_parameters_input.size());
  ...
  // time format info
  if (time_format_string == "Second" || time_format_string == "second") {
...
void EndEffectorIdentificationMomentumPybindWrapper::set_ipopt_parameters(
    const double tol, const double max_wall_time, const int print_level,
    const int max_iter, const std::string mu_strategy,
    const std::string linear_solver, const bool gradient_check) {
...
void EndEffectorIdentificationMomentumPybindWrapper::add_trajectory_file(
    const std::string filename_input) {
  try {
    auto start = std::chrono::high_resolution_clock::now();

    mynlp->add_trajectory_file(filename_input, sensor_noise, H, time_format,
                               downsample_rate);

    auto end = std::chrono::high_resolution_clock::now();
    std::cout << "Regressor computation time: "
    ...
    throw std::runtime_error(
        "Error adding trajectory file! Check previous error message!");
  }
}
...
nb::tuple EndEffectorIdentificationMomentumPybindWrapper::optimize() {
  ...
  try {
    ...
    solve_time =
        std::chrono::duration_cast<std::chrono::milliseconds>(end - start)
            .count();
    std::cout << "System identification solve time: " << solve_time
              << " milliseconds.\n";
  } catch (std::exception &e) {
    throw std::runtime_error(
  ...
  std::cout << "    theta solution: " << mynlp->theta_solution.transpose()
            << std::endl;
  std::cout << "    theta uncertainties: "
            << mynlp->theta_uncertainty.transpose() << std::endl;

  const size_t shape_ptr[] = {10};
  auto result1 = nb::ndarray<nb::numpy, const double>(
      mynlp->theta_solution.data(), 1, shape_ptr, nb::handle());

  auto result2 = nb::ndarray<nb::numpy, const double>(
      mynlp->theta_uncertainty.data(), 1, shape_ptr, nb::handle());

  return nb::make_tuple(result1, result2);
}

void EndEffectorIdentificationMomentumPybindWrapper::reset() { mynlp->reset(); }

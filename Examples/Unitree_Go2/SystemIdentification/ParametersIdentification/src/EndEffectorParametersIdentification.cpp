...
// This is the end effector system identification? how is it different to work
// from system identification in the Kinematic Dynamics???
namespace RAPTOR {
...
bool EndEffectorParametersIdentification::set_parameters(
  ...
  std::cout << "Go2 leg inertial parameters from URDF file: "
...
void EndEffectorParametersIdentification::add_trajectory_file(
  ...
  if (trajPtrs_.back()->Nact != modelPtr_->nv) {
    throw std::invalid_argument("The number of active joints in the trajectory "
                                "does not match the robot model.");
  }

  accelerations_.push_back(acceleration_input);

  if (accelerations_.back().rows() != trajPtrs_.back()->N) {
    throw std::invalid_argument("The number of rows in the acceleration file "
                                "does not match the trajectory.");
  }

  if (accelerations_.back().cols() != modelPtr_->nv) {
    throw std::invalid_argument("The number of columns in the acceleration "
                                "file does not match the robot model.");
  }

  initialize_regressors(trajPtrs_.back(), accelerations_.back());
...
void EndEffectorParametersIdentification::add_trajectory_file(
    ...
    const std::string acceleration_filename_input, const TimeFormat time_format,
    const int downsample_rate) {
  trajPtrs_.push_back(std::make_shared<TrajectoryData>(
      hardware_trajectory_filename_input, SensorNoiseInfo(modelPtr_->nv),
      time_format, downsample_rate));
  ...
  accelerations_.push_back(
      Utils::initializeEigenMatrixFromFile(acceleration_filename_input));
...
void EndEffectorParametersIdentification::initialize_regressors(
    const std::shared_ptr<TrajectoryData> &trajPtr_, const MatX &acceleration) {
  MatX A_seg_i = MatX::Zero(modelPtr_->nv * trajPtr_->N, 10 * modelPtr_->nv);
  VecX b_seg_i = VecX::Zero(modelPtr_->nv * trajPtr_->N);

  Index i = 0;
  // #pragma omp parallel for shared(modelPtr_, dataPtr_, trajPtr_, A_seg_i,
  // b_seg_i) private(i) schedule(dynamic, 1)
  for (i = 0; i < trajPtr_->N; i++) {
    // the following might be really confusing, but trajPtr_->q_dd stores the
    // torque data from the hardware and acceleration stores the actual
    // acceleration data from a separate file
    ...
    b_seg_i.segment(i * modelPtr_->nv, modelPtr_->nv) =
        ...
        modelPtr_->armature.cwiseProduct(q_dd) - offset;
  }
...
void EndEffectorParametersIdentification::reset() {
  ...
  std::cout << "End effector identification reset, all trajectory data removed."
            << std::endl;
}
...
bool EndEffectorParametersIdentification::eval_f(Index n, const Number *x,
                                                 bool new_x,
                                                 Number &obj_value) {
  if (n != numVars) {
...
bool EndEffectorParametersIdentification::eval_grad_f(Index n, const Number *x,
                                                      bool new_x,
                                                      Number *grad_f) {
  if (n != numVars) {
    THROW_EXCEPTION(IpoptException,
...
bool EndEffectorParametersIdentification::eval_hess_f(Index n, const Number *x,
                                                      bool new_x,
                                                      MatX &hess_f) {
  if (n != numVars) {
    THROW_EXCEPTION(IpoptException,
                    "*** Error wrong value of n in eval_hess_f!");
  }
...
EndEffectorParametersIdentification::z_to_theta(const VecX &z) {
  ...
  theta[2] = t2;
  theta[3] = t3;
  theta[4] = std::pow(s23, 2) + std::pow(t2, 2) + std::pow(t3, 2) +
             std::pow(exp_d2, 2) + std::pow(exp_d3, 2);
  theta[5] = -s12 * exp_d2 - s13 * s23 - t1 * t2;
  theta[6] = std::pow(s12, 2) + std::pow(s13, 2) + std::pow(t1, 2) +
             std::pow(t3, 2) + std::pow(exp_d1, 2) + std::pow(exp_d3, 2);
  theta[7] = -s13 * exp_d3 - t1 * t3;
  theta[8] = -s23 * exp_d3 - t2 * t3;
  theta[9] = std::pow(s12, 2) + std::pow(s13, 2) + std::pow(s23, 2) +
             std::pow(t1, 2) + std::pow(t2, 2) + std::pow(exp_d1, 2) +
             std::pow(exp_d2, 2);
...
Eigen::Vector<double, 10>
EndEffectorParametersIdentification::d_z_to_theta(const VecX &z,
                                                  Mat10 &dtheta) {
  const double alpha = z[0];
  const double d1 = z[1];
  const double d2 = z[2];
  ...
  dtheta(4, 0) = 2 *
                 (std::pow(s23, 2) + std::pow(t2, 2) + std::pow(t3, 2) +
                  exp_2d2 + exp_2d3) *
                 exp_2alpha;
  ...
  dtheta(6, 0) = 2 *
                 (std::pow(s12, 2) + std::pow(s13, 2) + std::pow(t1, 2) +
                  std::pow(t3, 2) + exp_2d1 + exp_2d3) *
                 exp_2alpha;
  ...
  dtheta(9, 0) = 2 *
                 (std::pow(s12, 2) + std::pow(s13, 2) + std::pow(s23, 2) +
                  std::pow(t1, 2) + std::pow(t2, 2) + exp_2d1 + exp_2d2) *
                 exp_2alpha;
...
Eigen::Vector<double, 10> EndEffectorParametersIdentification::dd_z_to_theta(
    const VecX &z, Mat10 &dtheta, Eigen::Array<Mat10, 1, 10> &ddtheta) {
  const double alpha = z[0];
  const double d1 = z[1];
  const double d2 = z[2];
  const double d3 = z[3];

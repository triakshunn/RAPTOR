...
public:
  ...
  // (for example, the last element of the mass_uncertainty vector is the end
  // effector mass uncertainty, but will be ignored if end_effector_mass_lb and
  // end_effector_mass_ub are defined)
  ...
  double suction_force =
      0; // additional force applied to the end effector when using suction cup
  double mu = 0; // friction coefficient of the contact surface
  double contact_surface_radius =
      0; // radius of the contact surface (radius of the suction cup)
  ...
  std::vector<std::pair<size_t, size_t>>
      tc_begin_and_end; // index of the two spheres that consist a tapered
                        // capsule
  std::vector<std::pair<size_t, size_t>> self_collision_checks;

  RobotInfo() = default;

  RobotInfo(const std::string &urdf_filename, const std::string &yaml_filename);

  ~RobotInfo() = default;

  void change_endeffector_inertial_parameters(
      const double mass, const Vec3 &com, const Vec6 &inertia,
      const double mass_eps, const double com_eps, const double inertia_eps);

  void
  change_endeffector_inertial_parameters(const Vec10 &inertial_parameters,
